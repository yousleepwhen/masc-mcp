...
let save_institution ~fs (config : config) (inst : institution) =
  ...
  (try Eio.Path.mkdirs ~exists_ok:true ~perm:0o755 dir_path with
   | Eio.Io _ as e -> Log.Institution.warn "mkdirs %s: %s" dir (Printexc.to_string e));
        ...
        (fun (p : pattern) ->
           ...
           then (
             ...
             let new_rate =
               if success
               then
                 ((p.success_rate *. float_of_int p.usage_count) +. 1.0)
      ...
      List.map
        (fun (p : pattern) ->
           ...
           then
             ...
             then
               { p with
                 effectiveness_used = p.effectiveness_used + 1
               ; effectiveness_last_check = now
               }
             else
               { p with
                 effectiveness_unused = p.effectiveness_unused + 1
               ; effectiveness_last_check = now
               }
           else p)
        inst.memory.procedural
    ...
    let procedural =
      List.filter
        (fun (p : pattern) ->
           let total = p.effectiveness_used + p.effectiveness_unused in
           if total = 0
           then true (* keep patterns with no effectiveness data *)
  ...
  then (
    ...
    List.iter
      (fun (p : pattern) ->
         ...
         if List.length p.steps <= 3
         then List.iter (fun step -> Printf.bprintf buf "     → %s\n" step) p.steps)
      sorted_patterns);
  (* Onboarding Steps *)
  Buffer.add_string buf "\n**Onboarding** (your first steps):\n";
  List.iteri
    (fun i step -> Printf.bprintf buf "  %d. %s\n" (i + 1) step)
...
let format_for_welcome (inst : institution) : string =
  ...
  let mission_short =
    ...
    then "(not defined)"
      ...
      String_util.utf8_safe ~max_bytes:80 ~suffix:"..." mission |> String_util.to_string
  in
...
let record_episode_jsonl ~event_type ~summary ~participants ~outcome ~learnings =
  ...
  (try Fs_compat.append_jsonl path (episode_to_json episode) with
   | Eio.Cancel.Cancelled _ as e -> raise e
   | exn ->
     Log.Institution.error "JSONL episode write failed: %s" (Printexc.to_string exn));
  episode
;;
