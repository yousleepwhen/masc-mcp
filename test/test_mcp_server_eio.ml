...
let test_handle_request_tools_list () =
  ...
  Alcotest.(check bool)
    "no masc_voice_speak in public surface"
    false
    (List.mem "masc_voice_speak" names);
  Alcotest.(check bool)
    "no masc_voice_ping_pong in public surface"
    false
            ...
            (match List.assoc_opt "tools" result_fields with
             | Some (`List tools) ->
                 ...
                 Alcotest.(check bool) "no managed voice agent" false
                   (List.mem "masc_voice_agent" names);
                 Alcotest.(check bool) "no managed voice speak" false
                   (List.mem "masc_voice_speak" names);
                 Alcotest.(check bool) "no managed voice ping_pong" false
                   (List.mem "masc_voice_ping_pong" names)
             | _ -> Alcotest.fail "tools not a list")
        | _ -> Alcotest.fail "result not an object")
   | _ -> Alcotest.fail "response not an object");
