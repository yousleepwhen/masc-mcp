...
let make_bundle
  ...
  let proof_contract_id =
    if contract_id_match then recomputed_contract_id else "md5:mismatched_hash"
  in
  let proof =
    make_proof
      ~run_id
      ~contract_id:proof_contract_id
...
let test_mode_propagation_violated () =
  ...
  Alcotest.(check bool) "has findings" true (List.length cr.findings > 0)
;;
...
let test_mode_escalation_violated () =
  ...
  let bundle =
    make_bundle
      ~proof_requested:Masc_mcp_cdal_runtime.Execution_mode.Draft
      ~proof_effective:Masc_mcp_cdal_runtime.Execution_mode.Execute
      ~contract_requested:Masc_mcp_cdal_runtime.Execution_mode.Draft
      ()
  in
  let cr = CJ.check_execution_mode bundle in
  Alcotest.(check string) "status" "violated" (CT.contract_status_to_string cr.status);
  let has_escalation =
    List.exists
      (fun (f : CT.contract_finding) -> f.event_id = Some "escalation")
      cr.findings
  in
...
let test_review_requirement_inconclusive_missing_evidence () =
  let bundle = make_bundle ?contract_review_requirement:(Some "human_review") () in
  let cr = CJ.check_review_requirement bundle in
  Alcotest.(check string) "status" "inconclusive" (CT.contract_status_to_string cr.status);
...
let test_review_requirement_inconclusive_warning_only () =
  let bundle =
    make_bundle
      ...
      ~proof_raw_evidence_refs:
        [ "proof-store://judge-test-001/evidence/review_warning.json" ]
      ()
  in
  let cr = CJ.check_review_requirement bundle in
  Alcotest.(check string) "status" "inconclusive" (CT.contract_status_to_string cr.status);
  let gap = List.hd cr.completeness_gaps in
  Alcotest.(check bool)
    "mentions warning-style evidence"
    true
...
let test_verdict_priority_inconclusive () =
  ...
  Alcotest.(check string)
    "status satisfied when no violations"
    "satisfied"
    (CT.contract_status_to_string verdict.status)
;;
...
let test_claim_scope_always_set () =
  ...
  Alcotest.(check string)
    "claim_scope ok"
    "phase1_scoped_runtime_audit"
    verdict_ok.claim_scope;
  (* Violated case *)
  let bundle_bad =
    make_bundle
      ~proof_risk:Masc_mcp_cdal_runtime.Risk_class.High
  ...
  Alcotest.(check string)
    "claim_scope violated"
    "phase1_scoped_runtime_audit"
    verdict_bad.claim_scope
;;
...
let test_loader_metadata_set () =
  ...
  Alcotest.(check string)
    ...
    verdict.loader_semantics_version;
  Alcotest.(check string)
    "schema compat mode"
    CT.schema_compat_mode_v1
    verdict.schema_compat_mode
;;
...
let test_replay_determinism () =
  ...
  Alcotest.(check string) "judgment_hash deterministic" v1.judgment_hash v2.judgment_hash;
  Alcotest.(check string)
    "judgment_basis_hash deterministic"
    v1.judgment_basis_hash
    v2.judgment_basis_hash
;;
...
let test_findings_populated () =
  ...
  Alcotest.(check string)
    ...
    "violated"
    (CT.contract_status_to_string verdict.status);
  Alcotest.(check bool) "has findings" true (List.length verdict.findings > 0);
  (* Verify finding has expected structure *)
  let f = List.hd verdict.findings in
  Alcotest.(check string) "finding check_id" "runtime.risk_class" f.check_id
;;
  ...
  Alcotest.run
    ...
    [ ( "checks"
      , [ Alcotest.test_case "all satisfied" `Quick test_all_satisfied
        ; Alcotest.test_case
            "mode propagation violated"
            `Quick
            test_mode_propagation_violated
        ; Alcotest.test_case
            "mode escalation violated"
            `Quick
            test_mode_escalation_violated
        ; Alcotest.test_case "risk class violated" `Quick test_risk_class_violated
        ; Alcotest.test_case
            "contract snapshot violated"
            `Quick
            test_contract_snapshot_violated
        ; Alcotest.test_case
            ...
            test_required_artifact_satisfied
        ; Alcotest.test_case
            "review requirement missing evidence"
            `Quick
            test_review_requirement_inconclusive_missing_evidence
        ; Alcotest.test_case
    ...
    ; ( "verdict"
      , [ Alcotest.test_case "violated wins" `Quick test_verdict_priority_violated_wins
        ; Alcotest.test_case "inconclusive path" `Quick test_verdict_priority_inconclusive
        ; Alcotest.test_case "claim scope always set" `Quick test_claim_scope_always_set
