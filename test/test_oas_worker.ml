...
let test_sdk_error_is_hard_quota_keeps_not_found_style_invalid_request_false () =
  let err =
    Oas.Error.Api
      (Llm_provider.Retry.InvalidRequest
         { message = "model endpoint not found" })
  in
  Alcotest.(check bool) "not-found InvalidRequest stays non-hard-quota" false
    (Oas_worker_named.sdk_error_is_hard_quota err)

let test_sdk_error_to_cascade_outcome_maps_not_found_style_invalid_request_to_404 () =
  let err =
    Oas.Error.Api
      (Llm_provider.Retry.InvalidRequest
         { message = "provider resource missing: not found" })
  in
  match Oas_worker_named.sdk_error_to_cascade_outcome err with
  | Some
      (Cascade_fsm.Call_err
         (Llm_provider.Http_client.HttpError
            { code = 404; body = "provider resource missing: not found" })) -> ()
  | outcome ->
      Alcotest.failf
        "expected Some (Call_err (HttpError 404)) for not-found InvalidRequest, got %s"
         (match outcome with
...
let () =
  ...
  Alcotest.run "OAS Worker" [
    ...
    "cascade_config", [
      ...
      Alcotest.test_case
        "sdk_error_is_hard_quota keeps not-found InvalidRequest false" `Quick
        test_sdk_error_is_hard_quota_keeps_not_found_style_invalid_request_false;
      Alcotest.test_case
        "sdk_error_to_cascade_outcome maps not-found InvalidRequest to 404" `Quick
        test_sdk_error_to_cascade_outcome_maps_not_found_style_invalid_request_to_404;
