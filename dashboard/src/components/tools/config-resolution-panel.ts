  ...
  return html`
    <div class="mt-4 rounded border border-[var(--color-border-default)] bg-[var(--white-3)] px-4 py-4">
      ...
      ${!state.value.error && !probe
        ? html`
            <div class="text-xs text-[var(--color-fg-muted)]" role=${state.value.loading ? 'status' : undefined}>
  ...
  return html`
    <${Card} title="설정 경로" class="section mb-4">
      <div class="mb-4 text-xs leading-relaxed text-[var(--color-fg-muted)]">
        서버가 실제로 해석한 config root와 runtime/data root를 함께 보여줍니다. 캐스케이드는 human-authored cascade.toml과 runtime cascade.json을 분리해 보여주며, 현재 실행이 바라보는 경로와 체크인된 seed config는 다를 수 있습니다.
      </div>
                ...
                <div class="flex flex-col gap-3">
                  ${runtimeResolution.diagnostics.length === 0
                    ...
                    : isFilteringDiagnostics && visibleDiagnostics.length === 0
                      ? html`
                          <div class="rounded border border-[var(--color-border-default)] bg-[var(--white-3)] px-3 py-3 text-center text-xs text-[var(--color-fg-muted)]" role="status" aria-live="polite">
