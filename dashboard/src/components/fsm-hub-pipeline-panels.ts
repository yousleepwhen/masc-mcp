...
const STATE_DESCRIPTIONS: Record<string, string> = {
  ...
  // KCL (Cascade)
  selecting: 'cascade 목록에서 최적 프로바이더 선택 중',
  trying: '선택된 프로바이더 로 inference 시도 중',
  done: '프로바이더 가 정상 응답함',
  exhausted: 'cascade 의 모든 프로바이더 실패',
  // KMC (Compaction)
  accumulating: '메시지 수집 중; 컨텍스트 아직 가득 차지 않음',
  // KSM (Phase) — used in Hero
  Running: '키퍼가 활성 상태로 턴 실행 중',
  Overflowed: '프롬프트가 프로바이더 컨텍스트 윈도우 초과; 복구 대기',
  ...
  return html`
    ...
    >
      <div class="flex items-baseline justify-between">
        <div>
          <div class="text-3xs font-semibold tracking-[0.06em] text-[var(--color-fg-muted)]" id="ksm-label">키퍼 생명주기 <span class="font-mono text-3xs text-[var(--color-fg-disabled)]">KSM</span></div>
      ...
      <div class=${`flex-1 rounded border px-3 py-2 transition-all duration-500 ${borderCls} ${bgCls} ${limited && !isActive ? 'opacity-60' : ''}`}>
        <div class="flex items-center justify-between gap-1.5">
          <div class="flex items-center gap-1.5 min-w-0">
            ${isActive ? html`<span class="h-1.5 w-1.5 rounded-full bg-[var(--indigo)] ${activePulse} shrink-0" aria-hidden="true"></span>` : null}
