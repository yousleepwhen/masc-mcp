                ...
                ${segments.map((seg, segIndex) => {
                  ...
                  return html`
                    <button
                      ...
                      class=${`${swimlaneSegmentColor(seg.value)} h-full transition-all duration-200 border-r border-[rgba(0,0,0,0.25)] last:border-r-0 cursor-pointer focus-visible:ring-2 focus-visible:ring-[var(--color-accent-fg)] focus-visible:ring-inset ${isHovered ? 'ring-1 ring-[var(--color-accent-fg)] brightness-125' : ''} ${dimmed ? 'opacity-40' : ''}`}
              ...
              <div class="flex flex-col gap-px">
                ${lane.entries.map((entry) => {
                  ...
                  return html`
                    ...
                    >
                      <span class="w-15 shrink-0 text-[var(--color-fg-primary)] truncate" title=${displayState(entry.value)}>${displayState(entry.value)}</span>
