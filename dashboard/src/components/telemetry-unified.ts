          ...
          <div class="rounded bg-[var(--black-20)] px-2 py-1.5">
            <div class="flex items-start justify-between gap-2">
              <details class="min-w-0 flex-1">
                <summary class="cursor-pointer text-3xs text-[var(--color-fg-disabled)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--color-accent-fg)]">원본 JSON</summary>
  ...
  return html`
    <div class="flex flex-col gap-4">
      ...
      <div class="flex items-center gap-3 flex-wrap">
        ...
        ${loading ? html`<span class="text-xs text-[var(--color-fg-muted)]" role="status">불러오는 중...</span>` : null}
      </div>
        ...
        <div class="max-h-150 overflow-y-auto" role="region" aria-label="텔레메트리 항목">
          ${displayItems.length > 0
            ...
            : isFilteringEntries && allDisplayItems.length > 0
              ? html`<div class="px-4 py-6 text-sm text-[var(--color-fg-muted)]" role="status" aria-live="polite">필터 결과 없음 (${allDisplayItems.length}건)</div>`
