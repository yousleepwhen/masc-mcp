...
function StatusBar({
  ...
  const liveBadge = snapshot
    ? snapshot.is_live
      ? html`<span role="status" class="px-2 py-0.5 rounded-sm border text-3xs font-mono text-[var(--color-status-ok)] border-[var(--ok-20)] bg-[var(--ok-10)] animate-pulse">● 실행 중</span>`
    ...
    <div class=${`sticky top-0 z-20 rounded border border-[var(--white-8)] bg-[var(--panel-dark-60)] backdrop-blur-sm shadow-[0_4px_12px_rgba(0,0,0,0.25)] ${containerPadding}`}>
      <div class="flex items-center justify-between gap-3 flex-wrap">
        <div class="flex items-center gap-3">
          ...
          ` : null}
          ${staleSec > 120 ? html`
            <span role="status" class="text-3xs font-mono text-[var(--bad-light)] animate-pulse" title="마지막 관측이 2분 이상 경과 — 대시보드 데이터가 현재 상태를 반영하지 않을 수 있습니다">
...
function SkeletonLayout() {
  return html`
    <div class="flex flex-col gap-3" aria-hidden="true" aria-busy="true">
...
function CollapsibleZone({
  ...
  return html`
    <div class="rounded border border-[var(--white-8)] bg-[var(--white-2)] overflow-hidden">
      <button
        type="button"
        class="w-full flex items-center justify-between px-4 py-2 text-left hover:bg-[var(--white-3)] transition-colors cursor-pointer select-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[var(--color-accent-fg)]"
        onClick=${toggle}
