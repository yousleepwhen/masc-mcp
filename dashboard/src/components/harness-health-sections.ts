  ...
  return html`
    <div class="space-y-2" role="list" aria-label="최근 판정 목록">
      ...
      ${isFiltering && visibleItems.length === 0
        ? html`<div class="py-4 text-center text-2xs text-[var(--color-fg-disabled)]" role="status" aria-live="polite">필터 결과 없음 (${items.length}건)</div>`
...
export function PreCompactList({ section }: { section: HarnessSignalSection<PreCompactEvent> }) {
  ...
  return html`
    <div class="space-y-2" role="list" aria-label="압축 이벤트 목록">
      ...
      ${isFiltering && visibleItems.length === 0
        ? html`<div class="py-4 text-center text-2xs text-[var(--color-fg-disabled)]" role="status" aria-live="polite">필터 결과 없음 (${section.recent_events.length}건)</div>`
        : visibleItems.map(item => html`
          <div class="rounded border border-[var(--white-8)] bg-[var(--white-4)] p-3" role="listitem">
            <div class="flex items-start justify-between gap-3">
