  ...
  return html`
    <div class="flex items-center gap-3 py-2 px-3 rounded border border-[var(--white-6)] bg-[var(--white-3)]" role="listitem" aria-label="${name}: ${getPhaseStyle(toPascalPhase(phase)).label}, 전환 ${transitions.length}건">
      <div class="w-24 shrink-0">
        <div class="text-sm font-semibold text-[var(--color-fg-secondary)] truncate" title=${name}>${name}</div>
        <div
        ...
        return d
          ...
          : html`
            <div class="flex items-center gap-3 py-2 px-3 rounded border border-[var(--white-6)] bg-[var(--white-3)]" role="listitem" key=${k.name}>
              <div class="w-24 text-sm font-semibold text-[var(--color-fg-secondary)] truncate" title=${k.name}>${k.name}</div>
