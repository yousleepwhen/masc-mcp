...
function CopyableToolCallBlock({
  ...
  return html`
    <div>
      ...
      <pre class=${`text-xs font-mono bg-[var(--bg-deep)] rounded p-2 overflow-x-auto ${maxHeightClass} whitespace-pre-wrap text-[var(--color-fg-secondary)]`} aria-label=${ariaLabel}>${value}</pre>
    </div>
  `
}
...
function ToolCallRow({ entry }: { entry: ToolCallEntry }) {
  ...
  return html`
    ...
    >
      <button
        type="button"
        class="w-full flex items-center gap-2 px-3 py-2 text-xs cursor-pointer text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
      ...
      >
        ...
        <span class=${`flex-shrink-0 w-5 text-center ${entry.success ? 'text-[var(--color-status-ok)]' : 'text-[var(--color-status-err)]'}`} aria-label=${entry.success ? '성공' : '실패'}>
          ${entry.success ? 'O' : 'X'}
        </span>
