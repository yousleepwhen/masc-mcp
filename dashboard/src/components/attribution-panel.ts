...
function EventRow({
  ...
  return html`
    ...
    >
      ...
      <span class="text-[var(--color-fg-muted)] truncate grow min-w-0" title=${reasonText || undefined}>
    ...
    <${SurfaceCard} variant="compact" ariaLabel="증거 상세">
      <div class="flex flex-col gap-3">
        ...
        ${reasonOf(a)
          ...
          : null}
        <pre class="text-2xs font-mono bg-[var(--white-5)]/30 rounded p-3 overflow-x-auto max-h-64 whitespace-pre-wrap" aria-label="증거 JSON">${evidenceJson}</pre>
      </div>
    </${SurfaceCard}>
  `
}
...
export function AttributionPanel() {
  ...
  if (loading.value && !summary.value) {
    return html`<${LoadingState} message="속성 정보 불러오는 중..." />`
  }
