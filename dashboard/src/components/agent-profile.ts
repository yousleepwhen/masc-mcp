      ...
      <div class="flex flex-col gap-1.5 min-w-0">
        ...
        ${ctxPct != null ? html`
          <div class="flex items-center gap-2 mt-0.5">
            ...
            <div class="h-1.5 mt-1.5 rounded-sm overflow-hidden bg-[var(--white-10)]" style="flex:1" aria-hidden="true">
      ...
      <div class="grid grid-cols-2 gap-4 mb-4">
        ...
        <${Card} title="타임라인" class="ff-card rounded">
          ${!timeline || (timeline.events ?? []).length === 0
            ...
            : html`<div class="flex flex-col gap-0.5 max-h-75 overflow-y-auto" role="log" aria-label="에이전트 타임라인">${(timeline.events ?? []).map((evt: AgentTimelineEvent, idx: number) => {
                ...
                return html`
                  <div class="flex flex-col gap-1.5">
                    ...
                    ${isFiltering && visible.length === 0
                      ? html`<div class="py-4 text-center text-2xs text-[var(--color-fg-disabled)]" role="status" aria-live="polite">필터 결과 없음 (${lines.length}건)</div>`
