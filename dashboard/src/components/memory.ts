...
function PostCard({ post }: { post: BoardPost }) {
  ...
  return html`
    <button
      type="button"
      class="board-post group w-full flex gap-3 rounded p-4 border border-[var(--color-border-default)] bg-[var(--card)] hover:bg-[var(--white-6)] hover:border-[var(--accent-20)] transition-all duration-200 cursor-pointer text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
  ...
  return html`
    <div>
      ...
      ${isFiltering && posts.length === 0 && rawPosts.length > 0
        ? html`<div class="py-4 text-center text-xs text-[var(--color-fg-disabled)]">필터 결과 없음 (${rawPosts.length}건)</div>`
