...
function HealthIndicator({ collapsed }: { collapsed?: boolean }) {
  ...
  const dot = html`<span class="block size-2 shrink-0 rounded-sm ${dotClass} shadow-[0_0_6px_rgba(0,0,0,0.4)]" aria-hidden="true"></span>`
            ...
            return html`
              <div class="flex flex-col gap-1">
                <${RouteLink}
                  ...
                  ariaCurrent=${isSurfaceActive ? 'true' : undefined}
                >
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="text-base font-medium truncate leading-none ${isSurfaceActive ? 'text-[var(--color-accent-fg)]' : ''}" title=${surface.label}>${surface.label}</div>
                  </div>
                <//>
