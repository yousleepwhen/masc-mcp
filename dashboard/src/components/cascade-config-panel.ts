...
function KeeperChip({ row }: { row: KeeperCascadeRow }) {
  ...
  const borderTone = row.drift
    ? 'border-[var(--warn)] text-[var(--color-fg-secondary)]'
    : 'border-[var(--card-border)] text-[var(--color-fg-secondary)]'
  return html`
    <span
            ...
            ${availableKeepers.length > 0
              ? html`
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    type="submit"
                    class="rounded border border-[var(--accent-primary)] bg-[var(--accent-primary)] px-3 py-1 text-xs font-medium text-white hover:opacity-90 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-fg)] focus-visible:ring-offset-1 focus-visible:ring-offset-[var(--color-bg-page)]"
              ...
              <li class="flex items-start gap-2 py-1 border-b border-[var(--card-border)] last:border-b-0">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  ${c.model !== displayModel
                    ? html`<div class="text-2xs text-[var(--color-fg-muted)] mt-0.5">config: <code>${c.model}</code></div>`
                    : null}
                  ${expanded.length > 1
                    ? html`
                      <ol class="mt-1 flex flex-col gap-0.5 text-2xs text-[var(--color-fg-muted)]" aria-label="확장 모델 목록">
  ...
  return html`
    <div class=${`rounded border ${boxTone} p-3 text-xs mb-3`}>
      ...
      ${visibleErrors.length > 0
        ? html`
          <ul class="flex flex-col gap-1 mb-2 text-[var(--color-fg-muted)]" aria-label="검증 오류 목록">
      ...
      ${visibleProfiles.length > 0
        ? html`
          ...
          <ul class="flex flex-col gap-1 text-[var(--color-fg-muted)]" aria-label="거부된 프로파일 목록">
...
function HealthTable({
  ...
  return html`
    ...
    ${isFiltering && filtered.length === 0
      ? html`<div class="py-4 text-center text-2xs text-[var(--color-fg-muted)]" role="status" aria-live="polite">필터 결과 없음 (${health.providers.length}건)</div>`
      : html`
      ...
      <div class="flex flex-col gap-3 p-4">
        ...
        <form class="flex flex-col gap-3" aria-label="Cascade 설정 편집" onSubmit=${handleSave}>
          ...
          <div class="flex items-center gap-3 flex-wrap">
            ...
            <button
              type="button"
              class="rounded border border-[var(--card-border)] bg-[var(--color-bg-page)] px-3 py-1 text-xs text-[var(--color-fg-secondary)] hover:bg-[var(--bg-panel-hover)] disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-fg)] focus-visible:ring-offset-1 focus-visible:ring-offset-[var(--color-bg-page)]"
              onClick=${handleReset}
  ...
  return html`
    <div class="flex flex-col gap-4">
      <div class="flex items-center gap-3 flex-wrap">
        <button
          type="button"
          class="rounded border border-[var(--card-border)] bg-[var(--color-bg-page)] px-3 py-1 text-xs text-[var(--color-fg-secondary)] hover:bg-[var(--bg-panel-hover)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-accent-fg)] focus-visible:ring-offset-1 focus-visible:ring-offset-[var(--color-bg-page)]"
