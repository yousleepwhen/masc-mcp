...
function PersonaCard({ persona }: { persona: PersonaSummary }) {
  ...
  return html`
    <div class="rounded border border-[var(--color-border-default)] bg-[var(--white-4)] p-4 flex flex-col gap-2 min-w-45">
      ...
      ${persona.description ? html`<div class="text-2xs text-[var(--color-fg-primary)] mt-1 line-clamp-2" title=${persona.description}>${persona.description}</div>` : null}
...
export function PersonaBrowser() {
  ...
  const spawnAccess = dashboardAuthAccess(shellAuthSummary.value, 'worker')
  if (personasLoading.value) return html`<p class="text-xs text-[var(--color-fg-muted)] py-4" role="status">페르소나 불러오는 중...</p>`
