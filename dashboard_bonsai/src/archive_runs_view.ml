...
let view_loop (l : Archive_runs_types.loop) =
  ...
  Node.div
    ~attrs:[ Style.loop; Attr.role "listitem"; Attr.arialabel (Archive_runs_types.status_label l.status ^ " · " ^ cycle_str ^ " · +" ^ Int.to_string keeps ^ " −" ^ Int.to_string discards ^ " · " ^ l.goal) ]
    [ Pill.view
